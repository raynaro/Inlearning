...
DEFAULT_ADMIN_EMAIL = os.environ.get('ADMIN_EMAIL', 'admin@idat.edu.pe').strip().lower()
DEFAULT_ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'admin123')
DEFAULT_OPERATIONAL_PASSWORD = os.environ.get('OPERATIONAL_DEFAULT_PASSWORD', 'Riva1234')
...
AREAS = ('SAE', 'COMERCIAL')
TV_AREAS = AREAS  # Cada usuario 'Pantalla TV' pertenece a un área: SAE o COMERCIAL.
COMMERCIAL_ACCOUNTS_PER_CAMPUS = 4
...
def consolidate_mixed_operational_users(cur):
    ...
    for name, email, role, account_code, campus in accounts:
        ...
        if row:
            ...
            cur.execute("INSERT INTO users(name,email,password,role,account_code,institute,campus,active,created_at) VALUES(?,?,?,?,?,?,?,1,?)",
                        (name, email, generate_password_hash(DEFAULT_OPERATIONAL_PASSWORD), role, account_code, None, campus, now_iso()))


def ensure_commercial_and_tv_accounts(cur):
    """Migración v4 (idempotente, no elimina ni reactiva nada).

    - Deja 4 usuarios Comercial por campus (Módulos 1 a 4).
    - Marca las pantallas TV existentes como TV SAE.
    - Crea una pantalla TV Comercial por campus, separada de la TV SAE.
    Solo inserta lo que falta; las cuentas ya existentes conservan contraseña y estado.
    """
    campuses = [('ATE', 'Lima Este'), ('SJM', 'Lima Sur'), ('SJL', 'Lima Norte')]
    cur.execute("UPDATE users SET tv_area='SAE' WHERE role='display' AND (tv_area IS NULL OR tv_area='')")
    for code, label in campuses:
        accounts = []
        for n in range(1, COMMERCIAL_ACCOUNTS_PER_CAMPUS + 1):
            accounts.append((f'Comercial {label} {n:02d}', f'comercial.{code.lower()}.{n:02d}@pdr.local',
                             'commercial', f'COM-{code}-{n:02d}', None))
        accounts.append((f'Pantalla TV Comercial {label}', f'pantalla.comercial.{code.lower()}@pdr.local',
                         'display', f'TVC-{code}-01', 'COMERCIAL'))
        for name, email, role, account_code, tv_area in accounts:
            row = cur.execute('SELECT id FROM users WHERE lower(email)=?', (email.lower(),)).fetchone()
            if row:
                if role == 'display':
                    cur.execute("UPDATE users SET tv_area=? WHERE id=?", (tv_area, row['id']))
                continue
            cur.execute("INSERT INTO users(name,email,password,role,account_code,institute,campus,active,tv_area,created_at) VALUES(?,?,?,?,?,?,?,1,?,?)",
                        (name, email, generate_password_hash(DEFAULT_OPERATIONAL_PASSWORD), role, account_code, None, code, tv_area, now_iso()))


def normalize_tv_area(value, default='SAE'):
    value = str(value or '').strip().upper()
    return value if value in TV_AREAS else default


def tv_area_label(value):
    return 'Comercial' if normalize_tv_area(value) == 'COMERCIAL' else 'SAE'


def display_user_area(user_id):
    """Área fija de una cuenta 'Pantalla TV' (SAE por defecto en cuentas antiguas)."""
    c = conn()
    row = c.execute('SELECT tv_area FROM users WHERE id=?', (user_id,)).fetchone()
    c.close()
    return normalize_tv_area(record_value(row, 'tv_area', 'SAE'))


def resolve_tv_area():
    """La TV de un usuario 'display' queda bloqueada a su área; solo el admin elige por URL."""
    if session.get('role') == 'display':
        return display_user_area(session.get('user_id'))
    return normalize_tv_area(request.args.get('area'))


def next_operational_account_code(c, role, campus, tv_area=None):
    prefix = {'security': 'SEG', 'sae': 'SAE', 'commercial': 'COM', 'display': 'TV'}.get(role, 'USR')
    if role == 'display' and normalize_tv_area(tv_area) == 'COMERCIAL':
        prefix = 'TVC'
...
def module_for_commercial_user(c, user_id):
    user = c.execute('SELECT id,role,campus,account_code,name FROM users WHERE id=?', (user_id,)).fetchone()
    if not user or record_value(user, 'role') != 'commercial' or normalize_campus(record_value(user, 'campus')) not in CAMPUSES:
        return ''
    campus = normalize_campus(record_value(user, 'campus'))
    rows = c.execute("SELECT id FROM users WHERE role='commercial' AND active=1 AND campus=? ORDER BY account_code,name,id", (campus,)).fetchall()
    for index, row in enumerate(rows, start=1):
        if record_value(row, 'id') == user_id:
            return f'Módulo {index}'
    return ''

def build_tv_payload(campus, area='SAE'):
    campus = normalize_campus(campus, 'SJM')
    area = area if area in ('SAE', 'COMERCIAL') else 'SAE'
    role_filter = 'sae' if area == 'SAE' else 'commercial'
    c = conn()
    staff_rows = c.execute("SELECT id,name,account_code,service_active FROM users WHERE role=? AND active=1 AND campus=? ORDER BY account_code,name,id", (role_filter, campus)).fetchall()
    staff = []
    for index, row in enumerate(staff_rows, start=1):
        staff.append({
            'id': record_value(row, 'id'),
            'name': record_value(row, 'name', ''),
            'module': f'Módulo {index}',
            'service_active': bool(int(record_value(row, 'service_active', 1) or 0)),
        })
    active_rows = c.execute("""SELECT v.*,cu.name current_user_name FROM visitors v
        LEFT JOIN users cu ON cu.id=v.current_sae_user_id
        WHERE v.campus=? AND v.destination_area=? AND v.attended=0
        AND COALESCE(v.flag_in_attention,0)=1 AND COALESCE(cu.service_active,0)=1
        ORDER BY COALESCE(v.service_module,''), COALESCE(v.status_updated_at,v.created_at) DESC, v.id DESC""", (campus, area)).fetchall()
    waiting_rows = c.execute("""SELECT v.* FROM visitors v WHERE v.campus=? AND v.destination_area=?
        AND v.attended=0 AND COALESCE(v.flag_in_attention,0)=0 AND COALESCE(v.flag_new,1)=1
        ORDER BY COALESCE(v.turn_number,v.id) ASC LIMIT 20""", (campus, area)).fetchall()
    served_today = c.execute("SELECT COUNT(*) n FROM visitors WHERE campus=? AND destination_area=? AND attended=1 AND substr(COALESCE(attended_at,created_at),1,10)=?", (campus, area, today_lima())).fetchone()['n']
    c.close()
    active_cards = []
    for index, row in enumerate(active_rows, start=1):
        active_cards.append({
            'id': record_value(row, 'id'),
            'turn_number': record_value(row, 'turn_number') or record_value(row, 'id'),
            'full_name': record_value(row, 'full_name', ''),
            'service_module': record_value(row, 'service_module', '') or f'Módulo {index}',
            'current_sae_name': record_value(row, 'current_user_name', '') or ('Asesor SAE' if area=='SAE' else 'Asesor Comercial'),
            'queue_order': index,
        })
    waiting_cards=[]
    ...
    return {
        'campus': campus, 'area': area, 'site_name': campus_label(campus),
        'updated_at': now_dt().strftime('%H:%M:%S'),
        'active_calls': active_cards, 'waiting': waiting_cards,
        'served_today': served_today, 'active_total': len(active_cards),
        'waiting_total': len(waiting_cards), 'staff': staff,
        'active_advisors': sum(1 for x in staff if x['service_active']),
        'total_advisors': len(staff), 'available_slots': max(sum(1 for x in staff if x['service_active'])-len(active_cards),0)
    }
...
def init_db():
    ...
    cur.execute(f'''CREATE TABLE IF NOT EXISTS users(
        ...
        active INTEGER NOT NULL DEFAULT 1, service_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL
    )''')
    ...
    cur.execute(f'''CREATE TABLE IF NOT EXISTS tv_videos(
        id {id_type}, filename TEXT NOT NULL, original_name TEXT NOT NULL,
        active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL
    )''')
    cur.execute(f'''CREATE TABLE IF NOT EXISTS tv_campus_videos(
        id {id_type}, campus TEXT NOT NULL, video_id INTEGER NOT NULL,
        position INTEGER NOT NULL DEFAULT 0
    )''')
    cur.execute(f'''CREATE TABLE IF NOT EXISTS tv_settings(
        campus TEXT PRIMARY KEY, transition_seconds INTEGER NOT NULL DEFAULT 1,
        transition_type TEXT NOT NULL DEFAULT 'fade', muted INTEGER NOT NULL DEFAULT 0,
        playback_mode TEXT NOT NULL DEFAULT 'random', single_video_id INTEGER
    )''')

    # Migraciones sobre proyectos anteriores.
    ensure_column(cur, 'tv_settings', 'playback_mode', "TEXT NOT NULL DEFAULT 'random'")
    ensure_column(cur, 'tv_settings', 'single_video_id', 'INTEGER')
    ...
    ensure_column(cur, 'users', 'service_active', 'INTEGER NOT NULL DEFAULT 1')
    ensure_column(cur, 'users', 'tv_area', 'TEXT')
    ...
    # Compatibilidad PostgreSQL/SQLite: estos campos son INTEGER, por lo que en
    # PostgreSQL no deben compararse con una cadena vacía ('').
    cur.execute("UPDATE visitors SET flag_new=1 WHERE flag_new IS NULL AND upper(visit_status)='NUEVA'")
    cur.execute("UPDATE visitors SET flag_new=0 WHERE flag_new IS NULL")
    cur.execute("UPDATE visitors SET flag_in_attention=1 WHERE flag_in_attention IS NULL AND upper(visit_status)='NO ATENDIDO'")
    ...
    cur.execute("UPDATE users SET service_active=1 WHERE service_active IS NULL")
    cur.execute("UPDATE visitors SET flag_new=0,visit_status='NO ATENDIDO' WHERE attended=0 AND COALESCE(flag_new,0)=1 AND COALESCE(flag_in_attention,0)=1")
    ...
    if not admin_row:
        cur.execute('''INSERT INTO users(name,email,password,role,account_code,institute,campus,active,created_at)
                       VALUES(?,?,?,?,?,?,?,1,?)''',
                    ('Administrador General', admin_email, generate_password_hash(DEFAULT_ADMIN_PASSWORD), 'admin', 'ADM-01', None, None, now_iso()))
    else:
    ...
    migration_v4 = cur.execute("SELECT value FROM app_meta WHERE key='inlearning_users_v4'").fetchone()
    if not migration_v4:
        ensure_commercial_and_tv_accounts(cur)
        cur.execute("INSERT INTO app_meta(key,value) VALUES(?,?)", ('inlearning_users_v4', now_iso()))
...
def build_activity_charts(c, area, institute='', campus=''):
    ...
    for days in (7, 30):
        # Los gráficos operativos trabajan de lunes a sábado. Domingo se excluye.
        desired_points = 6 if days == 7 else 30
        operational_dates = []
        cursor = end_date
        while len(operational_dates) < desired_points:
        ...
        for current in operational_dates:
...
@app.route('/health')
def health():
    return jsonify(status='ok', database='postgres' if USE_POSTGRES else 'sqlite'), 200
...
def register():
    if request.method == 'POST':
        ...
        if role == 'admin':
            campus = None
        tv_area = normalize_tv_area(request.form.get('tv_area')) if role == 'display' else None
        ...
        try:
            c = conn()
            account_code = request.form.get('account_code', '').strip() or (next_operational_account_code(c, role, campus, tv_area) if role != 'admin' else None)
            c.execute('''INSERT INTO users(name,email,password,role,account_code,institute,campus,active,tv_area,created_at)
                         VALUES(?,?,?,?,?,?,?,1,?,?)''',
                      (name, email, generate_password_hash(password), role, account_code, institute, campus, tv_area, now_iso()))
            ...
            if role == 'display':
                scope_text += f' · TV {tv_area_label(tv_area)}'
...
def users():
    ...
    c = conn()
    data = c.execute('''SELECT id,name,email,role,account_code,institute,campus,active,tv_area,created_at
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        if role != 'admin' and not account_code:
            account_code = next_operational_account_code(c, role, campus, tv_area)
        ...
        try:
            if password:
                c.execute('''UPDATE users SET name=?,email=?,role=?,account_code=?,institute=?,campus=?,active=?,tv_area=?,password=? WHERE id=?''',
                          (name, email, role, account_code, institute, campus, active, tv_area, generate_password_hash(password), user_id))
            else:
                c.execute('''UPDATE users SET name=?,email=?,role=?,account_code=?,institute=?,campus=?,active=?,tv_area=? WHERE id=?''',
                          (name, email, role, account_code, institute, campus, active, tv_area, user_id))
            c.commit(); c.close()
...
def visitors():
    ...
    for raw in raw_status:
        for item in str(raw).split(','):
            ...
            if value in ('NUEVA', 'ATENDIDO', 'NO ATENDIDO') and value not in status_filter:
                status_filter.append(value)
    base_conditions, base_params = [], []
    area_actual = ''
    sae_service_active = None
    sae_module = ''
    sae_busy = False
    if role == 'sae':
    ...
    sae_service_active = False
    sae_module = ''
    sae_busy = False
    if role == 'sae':
        user_row = c.execute('SELECT service_active FROM users WHERE id=?', (session['user_id'],)).fetchone()
        sae_service_active = bool(int(record_value(user_row, 'service_active', 1) or 0))
        sae_module = module_for_sae_user(c, session['user_id'])
        busy_row = c.execute("SELECT COUNT(*) n FROM visitors WHERE destination_area='SAE' AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1 AND current_sae_user_id=?", (campus, session['user_id'])).fetchone()
        sae_busy = int(busy_row['n'] or 0) > 0
    elif role == 'commercial':
        user_row = c.execute('SELECT service_active FROM users WHERE id=?', (session['user_id'],)).fetchone()
        sae_service_active = bool(int(record_value(user_row, 'service_active', 1) or 0))
        sae_module = module_for_commercial_user(c, session['user_id'])
        busy_row = c.execute("SELECT COUNT(*) n FROM visitors WHERE destination_area='COMERCIAL' AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1 AND current_sae_user_id=?", (campus, session['user_id'])).fetchone()
        sae_busy = int(busy_row['n'] or 0) > 0
    c.close()
    return render_template('visitors.html', visitors=data, area_actual=area_actual,
                           ...
                           sae_service_active=sae_service_active, sae_module=sae_module, sae_busy=sae_busy)
...
@app.route('/sae/disponibilidad', methods=['POST'])
@login_required
@role_required('sae')
def sae_availability():
    desired = 1 if request.form.get('active') == '1' else 0
    c = conn()
    if desired == 0:
        _, campus = session_scope()
        busy = c.execute("SELECT COUNT(*) n FROM visitors WHERE destination_area='SAE' AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1 AND current_sae_user_id=?", (campus, session['user_id'])).fetchone()['n']
        if int(busy or 0) > 0:
            c.close()
            flash('Finaliza la atención actual antes de ponerte inactivo.', 'danger')
            return redirect(url_for('visitors'))
    c.execute('UPDATE users SET service_active=? WHERE id=? AND role=?', (desired, session['user_id'], 'sae'))
    c.commit(); c.close()
    audit('DISPONIBILIDAD_SAE', 'user', session['user_id'], 'ACTIVO' if desired else 'INACTIVO')
    flash('Estado de atención SAE actualizado.', 'success')
    return redirect(url_for('visitors'))




@app.route('/comercial/disponibilidad', methods=['POST'])
@login_required
@role_required('commercial')
def commercial_availability():
    desired = 1 if request.form.get('active') == '1' else 0
    c = conn()
    if desired == 0:
        _, campus = session_scope()
        busy = c.execute("""SELECT COUNT(*) n FROM visitors WHERE destination_area='COMERCIAL'
            AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1
            AND current_sae_user_id=?""", (campus, session['user_id'])).fetchone()['n']
        if int(busy or 0) > 0:
            c.close()
            flash('Finaliza la atención actual antes de ponerte inactivo.', 'danger')
            return redirect(url_for('visitors', area='COMERCIAL'))
    c.execute("UPDATE users SET service_active=? WHERE id=? AND role='commercial'", (desired, session['user_id']))
    c.commit(); c.close()
    audit('DISPONIBILIDAD_COMERCIAL', 'user', session['user_id'], 'ACTIVO' if desired else 'INACTIVO')
    flash('Estado de atención Comercial actualizado.', 'success')
    return redirect(url_for('visitors', area='COMERCIAL'))
...
def update_visit_status(visit_id):
    ...
    if role == 'sae' and visit['destination_area'] == 'SAE' and estado == 'NO ATENDIDO':
        user_row = c.execute('SELECT service_active FROM users WHERE id=?', (session['user_id'],)).fetchone()
        if not user_row or int(record_value(user_row, 'service_active', 0) or 0) != 1:
            c.close(); flash('Tu módulo SAE está inactivo. Actívalo antes de tomar una visita.', 'danger'); return redirect(url_for('visitors'))
    if role == 'commercial' and visit['destination_area'] == 'COMERCIAL' and estado == 'NO ATENDIDO':
        user_row = c.execute('SELECT service_active FROM users WHERE id=?', (session['user_id'],)).fetchone()
        if not user_row or int(record_value(user_row, 'service_active', 0) or 0) != 1:
            c.close(); flash('Tu módulo Comercial está inactivo. Actívalo antes de tomar una visita.', 'danger'); return redirect(url_for('visitors', area='COMERCIAL'))
        busy = c.execute("SELECT COUNT(*) n FROM visitors WHERE destination_area='COMERCIAL' AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1 AND current_sae_user_id=? AND id<>?", (visit['campus'], session['user_id'], visit_id)).fetchone()['n']
        if int(busy or 0) > 0:
            c.close(); flash('Ya tienes un visitante en atención. Finalízalo antes de llamar al siguiente turno.', 'danger'); return redirect(url_for('visitors', area='COMERCIAL'))
    ...
    if attended:
        ...
        attended_at = now_iso()
        if visit['destination_area'] == 'SAE' and role == 'sae':
            current_sae_user_id = session['user_id']
            service_module = module_for_sae_user(c, session['user_id']) or service_module
        if visit['destination_area'] == 'COMERCIAL' and role == 'commercial':
            current_sae_user_id = session['user_id']
            service_module = module_for_commercial_user(c, session['user_id']) or service_module
    else:
        attended_by = None
        attended_at = None
        if role == 'sae' and visit['destination_area'] == 'SAE' and estado == 'NO ATENDIDO':
            current_sae_user_id = session['user_id']
            service_module = module_for_sae_user(c, session['user_id']) or service_module
        elif role == 'commercial' and visit['destination_area'] == 'COMERCIAL' and estado == 'NO ATENDIDO':
            current_sae_user_id = session['user_id']
            service_module = module_for_commercial_user(c, session['user_id']) or service_module
        elif estado == 'NUEVA':
    ...
    return_status = request.form.get('return_status', '').strip()
    redirect_args = {'status': return_status} if return_status else {}
    if role == 'admin':
...
def tv_video_config(campus):
    campus = normalize_campus(campus, 'SJM')
    c = conn()
    videos = c.execute("""SELECT v.id,v.filename,v.original_name,cv.position
                          FROM tv_campus_videos cv JOIN tv_videos v ON v.id=cv.video_id
                          WHERE cv.campus=? AND v.active=1 ORDER BY cv.position,cv.id""", (campus,)).fetchall()
    settings = c.execute('SELECT * FROM tv_settings WHERE campus=?', (campus,)).fetchone()
    mode = settings['playback_mode'] if settings and settings['playback_mode'] in ('single', 'random') else 'random'
    single_id = settings['single_video_id'] if settings else None
    items = [{'id': r['id'], 'name': r['original_name'], 'url': url_for('tv_video_file', filename=r['filename'])} for r in videos]
    if mode == 'single' and single_id:
        chosen = [v for v in items if v['id'] == single_id]
        items = chosen[:1]
    c.close()
    return {
        'videos': items,
        'playback_mode': mode,
        'single_video_id': single_id,
        'transition_seconds': 0.6,
        'transition_type': settings['transition_type'] if settings else 'fade',
        'muted': bool(settings['muted']) if settings else False,
    }
...
def tv_admin():
    campus = normalize_campus(request.args.get('campus'), 'SJM')
    c = conn()
    videos = c.execute('SELECT * FROM tv_videos ORDER BY created_at DESC,id DESC').fetchall()
    selected = c.execute('SELECT video_id FROM tv_campus_videos WHERE campus=? ORDER BY position,id', (campus,)).fetchall()
    settings = c.execute('SELECT * FROM tv_settings WHERE campus=?', (campus,)).fetchone()
    c.close()
    return render_template('tv_admin.html', campus=campus, videos=videos,
                           selected_ids=[r['video_id'] for r in selected], settings=settings)


@app.route('/admin/tv/subir', methods=['POST'])
@login_required
@admin_required
def tv_admin_upload():
    campus = normalize_campus(request.form.get('campus'), 'SJM')
    ...
    for file in files:
        ...
        if not original.lower().endswith('.mp4'):
            continue
        safe = secure_filename(original) or 'video.mp4'
        stamp = now_dt().strftime('%Y%m%d%H%M%S%f')
        filename = f'{stamp}_{safe}'
        file.save(os.path.join(TV_UPLOADS, filename))
        c.execute('INSERT INTO tv_videos(filename,original_name,active,created_at) VALUES(?,?,1,?)',
                  (filename, original, now_iso()))
        row = c.execute('SELECT id FROM tv_videos WHERE filename=? ORDER BY id DESC', (filename,)).fetchone()
        if row:
            maxpos = c.execute('SELECT MAX(position) AS p FROM tv_campus_videos WHERE campus=?', (campus,)).fetchone()
            pos = int((maxpos['p'] if maxpos and maxpos['p'] is not None else -1)) + 1
            c.execute('INSERT INTO tv_campus_videos(campus,video_id,position) VALUES(?,?,?)', (campus, row['id'], pos))
        saved += 1
    c.commit(); c.close()
    flash(f'{saved} video(s) MP4 importado(s).', 'success' if saved else 'danger')
    return redirect(url_for('tv_admin', campus=campus))


@app.route('/admin/tv/configurar', methods=['POST'])
@login_required
@admin_required
def tv_admin_configure():
    campus = normalize_campus(request.form.get('campus'), 'SJM')
    selected = []
    for value in request.form.getlist('video_ids'):
        try: selected.append(int(value))
        except ValueError: pass
    playback_mode = request.form.get('playback_mode', 'random')
    if playback_mode not in ('single', 'random'):
        playback_mode = 'random'
    try:
        single_video_id = int(request.form.get('single_video_id', '0')) or None
    except ValueError:
        single_video_id = None
    transition_type = request.form.get('transition_type', 'fade')
    if transition_type not in ('fade', 'cut'):
        transition_type = 'fade'
    muted = 1 if request.form.get('muted') == '1' else 0
    c = conn()
    # En modo de un solo video, garantizamos que el elegido esté asignado al campus.
    if playback_mode == 'single':
        exists = c.execute('SELECT id FROM tv_videos WHERE id=? AND active=1', (single_video_id,)).fetchone() if single_video_id else None
        if not exists:
            c.close()
            flash('Selecciona un video válido para repetir.', 'danger')
            return redirect(url_for('tv_admin', campus=campus))
        if single_video_id not in selected:
            selected.insert(0, single_video_id)
    elif not selected:
        c.close()
        flash('Selecciona al menos un video para la reproducción aleatoria.', 'danger')
        return redirect(url_for('tv_admin', campus=campus))
    c.execute('DELETE FROM tv_campus_videos WHERE campus=?', (campus,))
    for pos, vid in enumerate(selected):
        exists = c.execute('SELECT id FROM tv_videos WHERE id=? AND active=1', (vid,)).fetchone()
        if exists:
            c.execute('INSERT INTO tv_campus_videos(campus,video_id,position) VALUES(?,?,?)', (campus, vid, pos))
    c.execute('DELETE FROM tv_settings WHERE campus=?', (campus,))
    c.execute('INSERT INTO tv_settings(campus,transition_seconds,transition_type,muted,playback_mode,single_video_id) VALUES(?,?,?,?,?,?)',
              (campus, 1, transition_type, muted, playback_mode, single_video_id if playback_mode == 'single' else None))
    c.commit(); c.close()
    flash('Configuración de TV guardada para este campus.', 'success')
    return redirect(url_for('tv_admin', campus=campus))


@app.route('/admin/tv/video/<int:video_id>/eliminar', methods=['POST'])
@login_required
@admin_required
def tv_admin_delete_video(video_id):
    campus = normalize_campus(request.form.get('campus'), 'SJM')
    c = conn(); row = c.execute('SELECT filename FROM tv_videos WHERE id=?', (video_id,)).fetchone()
    if row:
        c.execute('DELETE FROM tv_campus_videos WHERE video_id=?', (video_id,))
        c.execute('DELETE FROM tv_videos WHERE id=?', (video_id,))
        c.commit()
        try: os.remove(os.path.join(TV_UPLOADS, row['filename']))
        except OSError: pass
...
@app.route('/tv-video/<path:filename>')
@login_required
@role_required('admin', 'display')
def tv_video_file(filename):
    return send_from_directory(TV_UPLOADS, filename, conditional=True)
...
def tv_board():
    ...
    area = resolve_tv_area()  # usuario TV: área fija de su cuenta; admin: ?area=
    payload = build_tv_payload(campus, area)
    tv_config = tv_video_config(campus)
    # Módulos = asesores activos del campus en esa área (SAE: 3, Comercial: 4). Respaldo si aún no hay cuentas.
    module_count = payload['total_advisors'] or (4 if area == 'COMERCIAL' else 3)
    return render_template('tv_board.html', payload=payload, tv_config=tv_config, campus=campus, area=area, module_count=module_count, admin_preview=session.get('role') == 'admin')
...
def tv_board_api():
    campus = normalize_campus(request.args.get('campus') if session.get('role') == 'admin' else session.get('campus'), 'SJM')
    area = resolve_tv_area()
    return jsonify(build_tv_payload(campus, area))
