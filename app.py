...
def brand_label(institute):
    ...
    return institute or 'IDAT + ZEGEL'
...
def branch_label(institute, campus):
    ...
    if institute and campus:
        return f'Marca {institute} · Sede {campus_label(campus)}'
    if campus:
        return f'Sede {campus_label(campus)} · IDAT + ZEGEL'
    if institute:
        return f'Marca {institute}'
    return 'Todas las marcas y sedes'
...
def inject_globals():
    return dict(roles=ROLES, areas=AREAS, institutes=INSTITUTES, brands=INSTITUTES, campuses=CAMPUSES, branches=BRANCHES, branch_campuses=BRANCH_CAMPUSES, branch_label=branch_label, site_label=campus_label, site_labels=SITE_LABELS)
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            if user['role'] != 'admin' and (not valid_branch(user['institute'], user['campus'])):
                flash('La cuenta todavía no tiene marca y sede asignadas. Comunícate con Administración.', 'danger')
...
def register():
    if request.method == 'POST':
        ...
        if role != 'admin' and not valid_branch(institute, campus):
            flash('Selecciona una combinación válida de marca y sede para la cuenta operativa.', 'danger')
            return render_template('register.html')
        try:
            c = conn()
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        if role != 'admin' and not valid_branch(institute, campus):
            c.close(); flash('Selecciona una combinación válida de marca y sede.', 'danger'); return render_template('edit_user.html', user=user)
...
def branches():
    ...
    for campus in CAMPUSES:
        ...
        card = {
            ...
            'pending': c.execute("SELECT COUNT(*) n FROM visitors WHERE campus=? AND visit_status='NUEVA'", (campus,)).fetchone()['n'],
...
def branch_detail(campus):
    ...
    if campus not in CAMPUSES:
        flash('Sede inválida.', 'danger'); return redirect(url_for('branches'))
    ...
    stats = {
        ...
        'visits_today': c.execute("SELECT COUNT(*) n FROM visitors WHERE campus=? AND substr(created_at,1,10)=?", (campus, today)).fetchone()['n'],
        'pending': c.execute("SELECT COUNT(*) n FROM visitors WHERE campus=? AND visit_status='NUEVA'", (campus,)).fetchone()['n'],
        'attended': c.execute("SELECT COUNT(*) n FROM visitors WHERE campus=? AND visit_status='ATENDIDO'", (campus,)).fetchone()['n'],
        ...
        'attended_sae': sum(1 for r in annual_rows if r['destination_area'] == 'SAE' and r['visit_status'] == 'ATENDIDO'),
        'attended_commercial': sum(1 for r in annual_rows if r['destination_area'] == 'COMERCIAL' and r['visit_status'] == 'ATENDIDO'),
    }
    users_data = c.execute('''SELECT * FROM users WHERE role<>'admin' AND campus=? ORDER BY role,institute,name''', (campus,)).fetchall()
...
def visitors():
    ...
    if role == 'sae':
        ...
        conditions.extend(['v.destination_area=?', 'v.campus=?'])
        params.extend([area_actual, campus])
...
def build_monthly_visit_report(month_value, institute='', campus='', area=''):
    ...
    summary.append(['Marca', institute or 'IDAT + ZEGEL'])
...
def dashboard():
    ...
    for sede in CAMPUSES:
        ...
        branch_cards.append({
            ...
            'brand_scope': institute or 'IDAT + ZEGEL',
