...
def dashboard():
    ...
    weekly_rows = c.execute("""SELECT substr(created_at,1,10) AS dia, COUNT(*) AS total,
                                      ...
                                      SUM(CASE WHEN destination_area='VENTAS' THEN 1 ELSE 0 END) AS ventas
                               FROM visitors
    ...
    for item in weekly:
        ...
        item['sae_pct'] = int((item['sae'] / max(1, item['total'])) * 100)
        item['ventas_pct'] = int((item['ventas'] / max(1, item['total'])) * 100)

    weekly_total = sum(d['total'] for d in weekly)
    weekly_goal = int(os.environ.get('WEEKLY_GOAL', '150') or 150)
    weekly_progress = min(100, int((weekly_total / max(1, weekly_goal)) * 100))
    ...
    return render_template('dashboard.html', stats=stats, recent=recent, recent_visitors=recent_visitors,
                           ...
                           weekly_total=weekly_total, weekly_goal=weekly_goal,
                           weekly_progress=weekly_progress, best_day=best_day,
