...
def build_tv_payload(campus):
    ...
    c = conn()
    staff_rows = c.execute("SELECT id,name,account_code,service_active FROM users WHERE role='sae' AND active=1 AND campus=? ORDER BY account_code,name,id", (campus,)).fetchall()
    staff = []
    ...
    active_rows = c.execute("SELECT v.*,cu.name current_sae_name FROM visitors v LEFT JOIN users cu ON cu.id=v.current_sae_user_id WHERE v.campus=? AND v.destination_area='SAE' AND v.attended=0 AND COALESCE(v.flag_in_attention,0)=1 AND COALESCE(cu.service_active,0)=1 ORDER BY COALESCE(v.service_module,''), COALESCE(v.status_updated_at,v.created_at) DESC, v.id DESC", (campus,)).fetchall()
    ...
    for index, row in enumerate(active_rows, start=1):
        active_cards.append({
            ...
            'current_sae_name': record_value(row, 'current_sae_name', '') or 'Asesor SAE',
            'queue_order': index,
        })
    waiting_cards = []
    ...
    active_advisors = sum(1 for item in staff if item['service_active'])
    return {
        'campus': campus,
        'site_name': campus_label(campus),
        'updated_at': now_dt().strftime('%H:%M:%S'),
        'active_calls': active_cards,
        'waiting': waiting_cards,
        'served_today': served_today,
        'active_total': len(active_cards),
        'waiting_total': len(waiting_cards),
        'sae_staff': staff,
        'active_advisors': active_advisors,
        'total_advisors': len(staff),
        'available_slots': max(active_advisors - len(active_cards), 0),
    }
...
def update_visit_status(visit_id):
    ...
    if role == 'sae' and visit['destination_area'] == 'SAE' and estado == 'NO ATENDIDO':
        ...
        busy = c.execute("SELECT COUNT(*) n FROM visitors WHERE destination_area='SAE' AND campus=? AND attended=0 AND COALESCE(flag_in_attention,0)=1 AND current_sae_user_id=? AND id<>?", (visit['campus'], session['user_id'], visit_id)).fetchone()['n']
        if int(busy or 0) > 0:
            c.close(); flash('Ya tienes un visitante en atención. Finalízalo antes de llamar al siguiente turno.', 'danger'); return redirect(url_for('visitors'))
...
def tv_board():
    ...
    return render_template('tv_board.html', payload=payload, tv_config=tv_config, campus=campus, admin_preview=session.get('role') == 'admin')
