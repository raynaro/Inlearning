...
def import_teachers_from_excel(path):
    ...
    if i_name is None or i_dni is None:
        raise ValueError('El Excel de docentes debe tener columnas: Docente/NombreCompleto y DNI. También puede incluir Usuario, Correo y Teléfono Docente.')
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        username = str(row[i_user] or '').strip().lower() if i_user is not None else ''
        email = str(row[i_email] or '').strip().lower() if i_email is not None else ''
        phone = str(row[i_phone] or '').strip() if i_phone is not None else ''
        if not name or not dni:
            continue
        cur.execute('''INSERT INTO teachers(name,dni,area,username,email,phone,active,created_at,updated_at)
                       VALUES(?,?,?,?,?,?,1,?,?)
                       ON CONFLICT(dni) DO UPDATE SET
                       name=excluded.name, area=excluded.area, username=excluded.username,
                       email=excluded.email, phone=excluded.phone, active=1, updated_at=excluded.updated_at''',
                    (name, dni, area, username, email, phone, now_iso(), now_iso()))
        count += 1
    c.commit(); c.close()
    return count
...
def init_db():
    ...
    # Migración automática: antes el módulo se llamaba Ventas/sales.
    # Ahora todo queda como Comercial/commercial sin perder registros previos.
    cur.execute("UPDATE users SET role='commercial' WHERE role='sales'")
    cur.execute("UPDATE users SET name='Personal Comercial', email='comercial@idat.edu.pe' WHERE email='ventas@idat.edu.pe'")
    cur.execute("UPDATE visitors SET destination_area='COMERCIAL' WHERE destination_area='VENTAS'")

    users = [
        ('Administrador', 'admin@idat.edu.pe', 'admin123', 'admin'),
        ('Personal SAE', 'sae@idat.edu.pe', 'sae123', 'sae'),
        ('Personal Comercial', 'comercial@idat.edu.pe', 'comercial123', 'commercial'),
    ...
    if total_teachers == 0 and os.path.exists(TEACHERS_XLSX):
        import_teachers_from_excel(TEACHERS_XLSX)
    purge_old_logs()
...
def security_panel():
    ...
    stats = {
        'students_today': c.execute("SELECT COUNT(*) AS n FROM access_logs WHERE user_id=? AND substr(created_at,1,10)=?", (session['user_id'], today)).fetchone()['n'],
        'teachers_today': c.execute("SELECT COUNT(*) AS n FROM teacher_logs WHERE user_id=? AND substr(created_at,1,10)=?", (session['user_id'], today)).fetchone()['n'],
        'visits_today': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE user_id=? AND substr(created_at,1,10)=?", (session['user_id'], today)).fetchone()['n'],
...
def new_visitor():
    if request.method == 'POST':
        ...
        if session.get('role') == 'sae':
            ...
            destination_area = 'COMERCIAL'
        reason = request.form.get('reason','').strip()
        if destination_area not in AREAS:
            flash('Selecciona el área SAE o COMERCIAL.', 'danger'); return render_template('visitor_form.html')
...
def visitors():
    ...
    if role == 'sae':
        ...
        area_actual = 'COMERCIAL'
        sonido_area = True
        data = c.execute('''SELECT v.*, u.name user FROM visitors v LEFT JOIN users u ON u.id=v.user_id
                            WHERE v.destination_area='COMERCIAL' ORDER BY v.id ASC''').fetchall()
    elif role == 'security':
        data = c.execute('''SELECT v.*, u.name user FROM visitors v LEFT JOIN users u ON u.id=v.user_id
    ...
    ultimo = data[-1]['id'] if data and role in ('sae','commercial') else 0
    c.close()
...
def visitas_estado():
    ...
    if area:
        row = c.execute("""SELECT COUNT(*) AS total,
                                  ...
                                  SUM(CASE WHEN attended=0 THEN 1 ELSE 0 END) AS pendientes,
                                  COALESCE(MAX(CASE WHEN attended=0 THEN id ELSE 0 END),0) AS ultimo_pendiente
                           FROM visitors WHERE destination_area=?""", (area,)).fetchone()
        nuevos = c.execute('SELECT COUNT(*) AS n FROM visitors WHERE destination_area=? AND id>?', (area, since_id)).fetchone()['n']
    else:
        row = c.execute("""SELECT COUNT(*) AS total,
                                  COALESCE(MAX(id),0) AS ultimo,
                                  SUM(CASE WHEN attended=0 THEN 1 ELSE 0 END) AS pendientes,
        ...
        nuevos = c.execute('SELECT COUNT(*) AS n FROM visitors WHERE id>?', (since_id,)).fetchone()['n']
    c.close()
    return jsonify({
...
def update_visit_status(visit_id):
    ...
    if role == 'commercial' and visit['destination_area'] != 'COMERCIAL':
        c.close(); flash('No tienes permiso para cambiar esta visita.', 'danger'); return redirect(url_for('visitors'))
...
def week_dates_lima(days=6):
    """Devuelve los dias de la semana actual de lunes a sabado (Lima)."""
    base = now_dt().date()
...
def dashboard():
    c=conn(); today=today_lima()
    week_days = week_dates_lima(6)
    stats = {
        'active': c.execute('SELECT COUNT(*) AS n FROM students WHERE active=1').fetchone()['n'],
        'teachers': c.execute('SELECT COUNT(*) AS n FROM teachers WHERE active=1').fetchone()['n'],
        'today': c.execute("SELECT COUNT(*) AS n FROM access_logs WHERE substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'visitors': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'sae_today': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE destination_area='SAE' AND substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'comercial_today': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE destination_area='COMERCIAL' AND substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'sae_attended_today': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE destination_area='SAE' AND attended=1 AND substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'comercial_attended_today': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE destination_area='COMERCIAL' AND attended=1 AND substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'users': c.execute('SELECT COUNT(*) AS n FROM users').fetchone()['n'],
        'inactive': c.execute("SELECT COUNT(*) AS n FROM access_logs WHERE result='INACTIVO'").fetchone()['n'],
        'visitors_week': c.execute("SELECT COUNT(*) AS n FROM visitors WHERE substr(created_at,1,10) BETWEEN ? AND ?", (week_days[0], week_days[-1])).fetchone()['n'],
        'pending': c.execute('SELECT COUNT(*) AS n FROM visitors WHERE attended=0').fetchone()['n'],
        'attended': c.execute('SELECT COUNT(*) AS n FROM visitors WHERE attended=1').fetchone()['n'],
        'students_active_today': c.execute("SELECT COUNT(*) AS n FROM access_logs WHERE result='ACTIVO' AND substr(created_at,1,10)=?", (today,)).fetchone()['n'],
        'teachers_active_today': c.execute("SELECT COUNT(*) AS n FROM teacher_logs WHERE result='ACTIVO' AND substr(created_at,1,10)=?", (today,)).fetchone()['n']
    }
    weekly_rows = c.execute("""SELECT substr(created_at,1,10) AS dia, COUNT(*) AS total,
                                      SUM(CASE WHEN destination_area='SAE' THEN 1 ELSE 0 END) AS sae,
                                      SUM(CASE WHEN destination_area='COMERCIAL' THEN 1 ELSE 0 END) AS comercial
                               FROM visitors
    ...
    for idx, d in enumerate(week_days):
        ...
        sae = r['sae'] if r else 0
        comercial = r['comercial'] if r else 0
        item = {
            'fecha': d,
            'dia': day_names[idx],
            'dia_corto': day_names[idx][:3],
            'total': total,
            'sae': sae,
            'comercial': comercial,
            'other': max(0, total - sae - comercial),
            'is_today': d == today
        }
        max_total = max(max_total, total)
        weekly.append(item)
    max_sae = max([d['sae'] for d in weekly] + [1])
    max_comercial = max([d['comercial'] for d in weekly] + [1])
    for item in weekly:
        item['pct'] = int((item['total'] / max_total) * 100) if max_total else 0
        item['sae_pct'] = int((item['sae'] / max_sae) * 100) if max_sae else 0
        item['comercial_pct'] = int((item['comercial'] / max_comercial) * 100) if max_comercial else 0

    weekly_total = sum(d['total'] for d in weekly)
    weekly_sae_total = sum(d['sae'] for d in weekly)
    weekly_comercial_total = sum(d['comercial'] for d in weekly)
    best_day = max(weekly, key=lambda d: d['total']) if weekly else {'dia': '-', 'total': 0}
    today_index = next((i for i, d in enumerate(weekly) if d['is_today']), len(weekly)-1)
    prev_total = weekly[today_index-1]['total'] if today_index > 0 else 0
    current_total = weekly[today_index]['total'] if weekly else 0
    if prev_total > 0:
        growth_pct = int(((current_total - prev_total) / prev_total) * 100)
        growth_label = f"{growth_pct:+d}% vs dia anterior"
    elif current_total > 0:
        growth_label = '+100% vs dia anterior'
    else:
        growth_label = 'Sin variacion aun'

    area_stats = c.execute("""SELECT destination_area AS area, COUNT(*) AS total,
                                     SUM(CASE WHEN attended=1 THEN 1 ELSE 0 END) AS atendidos,
                                     SUM(CASE WHEN attended=0 THEN 1 ELSE 0 END) AS pendientes
                              FROM visitors GROUP BY destination_area ORDER BY destination_area""").fetchall()
    max_area_total = max([a['total'] or 0 for a in area_stats] + [1])
    area_bars = []
    for a in area_stats:
        total = a['total'] or 0
        atendidos = a['atendidos'] or 0
        pendientes = a['pendientes'] or 0
        area_bars.append({
            'area': a['area'],
            'total': total,
            'atendidos': atendidos,
            'pendientes': pendientes,
            'pct': int((total / max_area_total) * 100) if max_area_total else 0,
            'pct_atendidos': int((atendidos / max(1, total)) * 100),
            'pct_pendientes': int((pendientes / max(1, total)) * 100)
        })
    ...
    commercial_detail = c.execute("""SELECT v.*, u.name user FROM visitors v LEFT JOIN users u ON u.id=v.user_id
                                WHERE v.destination_area='COMERCIAL' ORDER BY v.id DESC LIMIT 12""").fetchall()
    ...
    user_activity = c.execute("""
        ...
        FROM users u
        LEFT JOIN (SELECT user_id, COUNT(*) AS alumnos FROM access_logs GROUP BY user_id) a ON a.user_id=u.id
        LEFT JOIN (SELECT user_id, COUNT(*) AS docentes FROM teacher_logs GROUP BY user_id) t ON t.user_id=u.id
        LEFT JOIN (SELECT user_id, COUNT(*) AS visitas FROM visitors GROUP BY user_id) v ON v.user_id=u.id
    ...
    return render_template('dashboard.html', stats=stats, recent=recent, recent_visitors=recent_visitors,
                           ...
                           weekly_total=weekly_total, weekly_sae_total=weekly_sae_total,
                           weekly_comercial_total=weekly_comercial_total, best_day=best_day,
                           growth_label=growth_label, area_stats=area_stats,
                           area_bars=area_bars, user_activity=user_activity,
                           sae_detail=sae_detail, commercial_detail=commercial_detail, security_detail=security_detail,
...
def teachers():
    ...
    c=conn()
    if q:
        data=c.execute("SELECT * FROM teachers WHERE name LIKE ? OR dni LIKE ? OR area LIKE ? OR username LIKE ? OR email LIKE ? OR phone LIKE ? ORDER BY active DESC, name",
...
def style_sheet(ws):
    fill = PatternFill('solid', fgColor='F26B2F')
    for cell in ws[1]:
