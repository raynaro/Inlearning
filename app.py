...
def dashboard():
    ...
    for idx, item in enumerate(weekly):
        ...
        item['x'] = int((idx / max(1, len(weekly)-1)) * 100)
        item['y_total'] = int(92 - ((item['total'] / max_total) * 78)) if max_total else 92
        item['y_sae'] = int(92 - ((item['sae'] / max_total) * 78)) if max_total else 92
        item['y_ventas'] = int(92 - ((item['ventas'] / max_total) * 78)) if max_total else 92

    line_points_total = ' '.join([f"{d['x']},{d['y_total']}" for d in weekly])
    line_points_sae = ' '.join([f"{d['x']},{d['y_sae']}" for d in weekly])
    line_points_ventas = ' '.join([f"{d['x']},{d['y_ventas']}" for d in weekly])
    ...
    return render_template('dashboard.html', stats=stats, recent=recent, recent_visitors=recent_visitors,
                           ...
                           line_points_total=line_points_total, line_points_sae=line_points_sae,
                           line_points_ventas=line_points_ventas, area_stats=area_stats,
